...
public class GetParameter extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (productName.trim().equals("")) {
			out.print("<h1>ProductName沒有東西</h1>");
		} else {
		...
		if (productPrice==null) {
			out.print("<h1>ProductPrice沒有東西</h1>");
		} else {
		...
		if (gameTypeNo==null) {
			out.print("<h1>GameTypeNo沒有東西</h1>");
		} else {
		...
		if (itemProdDescription.trim().equals("")) {
			out.print("<h1>ItemProdDescription沒有東西</h1>");
		} else {
			out.print("<h1>" + itemProdDescription + "</h1>");
		}

		String upcNum = request.getParameter("UpcNum");
		if (upcNum.trim().equals("")) {
			out.print("<h1>GamePlatformNo沒有東西</h1>");
		} else {
			out.print("<h1>" + upcNum + "</h1>");
		}
		...
		try {
			String productNoCompare = (request.getParameter("ProductNo").equals("")?"0":request.getParameter("ProductNo"));
...
//					String update = "UPDATE product SET ProductName = ?, ProductPrice = ?, GameTypeNo = ?, GamePlatformNo = ?, GameCompanyNo = ?, ProductState = ?,LaunchedTime = now(), ItemProdDescription = ? WHERE (ProductNo = ?); ";
					...
					productService.updateProductAndSold(productNo,gameTypeNo, gamePlatformNo, gameCompanyNo, productName, productPrice, productState, itemProdDescription, upcNum);
...
//					更新圖片3
	...
	//重導			response.sendRedirect("backend/product/productUpAndMod.html");
					response.setHeader("Refresh","2;URL=http://localhost:8081/CGA101G1/backend/product/productMod.html");
...
//					String update = "UPDATE product SET ProductName = ?, ProductPrice = ?, GameTypeNo = ?, GamePlatformNo = ?, GameCompanyNo = ?, ProductState = ?,SoldTime = now(), ItemProdDescription = ? WHERE (ProductNo = ?); ";
					...
					ProductService productService = new ProductService();
					productService.updateProductNotSold(productNo, gameTypeNo, gamePlatformNo, gameCompanyNo, productName, productPrice, productState, itemProdDescription, upcNum);
...
//				判斷是否上架 未來要檢查有沒有是int
				if(request.getParameter("ProductState").equals("1")) {
					...
					ProductService productService = new ProductService();
					productService.AddNewProductAndSold(gameTypeNo, gamePlatformNo, gameCompanyNo, productName, productPrice, productState, itemProdDescription, upcNum);
...
//			insert產品圖3
	...
	//重導			response.sendRedirect("backend/product/productUpAndMod.html");
					response.setHeader("Refresh","2;URL=http://localhost:8081/CGA101G1/backend/product/productAdd.html");
				...
				}else 
				{
					/*--------------新增但是不上架--------------------*/
					ProductService productService = new ProductService();
					productService.AddNewProductNotSold(gameTypeNo, gamePlatformNo, gameCompanyNo, productName, productPrice, productState, itemProdDescription, upcNum);
